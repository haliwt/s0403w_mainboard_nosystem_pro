...
/**
 * @brief  :  static void vTaskStart(void *pvParameters)´´½¨Êý¾Ý´¦ÀíÈÎÎñ£¬ÓÅÏÈ¼¶ÎªÖÐµÈ
 * @note    ÈÎÎñÄÚ²¿Ê¹ÓÃ¶ÓÁÐ½ÓÊÕÊý¾Ý£¬ÐèÏÈ³õÊ¼»¯¶ÓÁÐ
 * @param   None
 * @retval  None
 */
static void vTaskMsgPro(void *pvParameters)
...
static void vTaskStart(void *pvParameters)
{
	...
	BaseType_t xResult;
	const TickType_t xMaxBlockTime = pdMS_TO_TICKS(2000); /* ÉèÖÃ×î´óµÈ´ýÊ±¼äÎª500ms */
	uint32_t ulValue;
	while(1)
    {
		...
		xResult = xTaskNotifyWait(0x00000000,      
						          0xFFFFFFFF,      
						          &ulValue,        /* ±£´æulNotifiedValueµ½±äÁ¿ulValueÖÐ */
...
/**
 * @brief  :  void AppTaskCreate (void)½¨Êý¾Ý´¦ÀíÈÎÎñ£¬ÓÅÏÈ¼¶ÎªÖÐµÈ
 * @note    ÈÎÎñÄÚ²¿Ê¹ÓÃ¶ÓÁÐ½ÓÊÕÊý¾Ý£¬ÐèÏÈ³õÊ¼»¯¶ÓÁÐ
 * @param   None
 * @retval  None
 */
